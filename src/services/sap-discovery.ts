...
export class SAPDiscoveryService {
    ...
    async discoverAllServices(): Promise<ODataService[]> {
        ...
        try {
            ...
            // Discover services directly from the configured destination
            const destinationServices = await this.discoverFromDestination();
            services.push(...destinationServices);
            ...
            for (const service of limitedServices) {
                try {
                    ...
                    const metadata = await this.getServiceMetadata(service);
                    service.metadata = metadata;

                    // Update service OData version and entity sets from metadata
                    if (metadata) {
                        service.odataVersion = (metadata.version === 'v4' ? 'v4' : 'v2') as 'v2' | 'v4';
                        service.entitySets = metadata.entitySets
                            .map((es: any) => es.name as string)
                            .filter((name: any) => name && typeof name === 'string');
                        this.logger.debug(`Service ${service.id} has ${service.entitySets.length} entity sets`);
                    }
                } catch (error) {
    ...
    private async discoverFromDestination(): Promise<ODataService[]> {
        try {
            ...
            this.logger.info(`Discovering service from destination: ${destination.url}`);

            // Create service entry for the destination service
            const service: ODataService = {
                id: 'SEPMRA_PROD_MAN',
                version: '0001',
                title: 'SEPMRA_PROD_MAN',
                description: `OData service from destination URL`,
                odataVersion: 'v2', // Assume V2 for now, will be determined from metadata
                url: destination.url,
                metadataUrl: `${destination.url}$metadata`,
                entitySets: [],
                metadata: null
            };

            this.logger.info(`Created service entry: ${service.id}`);
            return [service];

        } catch (error) {
            this.logger.error('Destination-based service discovery failed:', error);
            return [];
        }
    }
